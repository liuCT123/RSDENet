...
class Solver:
    def __init__(self, exp_name: str, arg_dict: dict, path_dict: dict):
        ...
        # self.tr_data_edge_path = self.arg_dict["rgb_data"]["tr_data_edge_path"]
        ...
        self.tr_loader = create_loader(
            ...
            # data_path2=self.tr_data_edge_path,
            training=True,
    ...
    def train(self):
        for curr_epoch in range(self.start_epoch, self.end_epoch): # 0--epoch-1
            ...
            if (curr_epoch + 1)% 20 == 0:
                torch.save(self.net.state_dict(), model_dir+ "\epoch_%d.pth" % (curr_epoch + 1))
            elif ((curr_epoch + 1) >= 85) and(curr_epoch + 1)% 5 == 0:
                torch.save(self.net.state_dict(), model_dir + "\epoch_%d.pth" % (curr_epoch + 1))
                # 保存参数

            if((curr_epoch+1) >= 100) and (curr_epoch + 1) % 5 == 0:
                self.test()  # 测试每个epoch
    ...
    def cal_hybridloss(self, pre, GT):
        ...
        # PPA_loss = structure_loss(pre,GT)
    ...
    def _train_per_epoch(self, curr_epoch, train_loss_record):
        for curr_iter_in_epoch, train_data in enumerate(self.tr_loader):
            ...
            # train_sal_edge = train_sal_edge.to(self.dev, non_blocking=True)
            # train_masks = train_masks.cuda()   #edit
            # s1, s2, s3, s4, s5 ,pose,auxiliary= self.net(train_inputs)
            # s2, s3, s4, s5 = self.net(train_inputs)
            #swin-s
            # s1, s2, s3, s4, s5,s6= self.net(train_inputs)
            #vgg/res
    ...
    def _test_process(self, save_pre):
        ...
        for test_batch_id, test_data in tqdm_iter:
            ...
            with torch.no_grad():
                ...
                # outputs, s2, s3, s4, s5,s6,s7,s8= self.net(in_imgs)  #resnet,vgg
                outputs, s2, s3, s4, s5= self.net(in_imgs)  #swin-s
                # outputs,s2, s3, s4, s5 ,s6= self.net(in_imgs)  #output就是s1
