...
SDI_te_path = os.path.join(datasets_root, "mydataset/Test/")

SDI_tr_path = "./dataset/mydataset/Train/"



arg_config = {
    "model": "RSDENet",  # 实际使用的模型，需要在`network/__init__.py`中导入   A3Net_VGG16  A3Net_Res2Net50  A3Net_Swins
    ...
    "epoch_num": 150,  # 训练周期, 0: directly test model   100
    "lr": 0.0015,  # 微调时缩小100倍  vgg 0.0015   swins/res2_50/res50 0.001
    "xlsx_name": "result.xlsx",  # the name of the record file
    # 数据集设置
    "rgb_data": {
        "tr_data_path": SDI_tr_path,
        # "tr_data_edge_path":SDI_tr_edge_path,
        "te_data_list": OrderedDict(
            {

                "test_result": SDI_te_path,
            },
        ),
    },
    # 训练过程中的监控信息
    ...
    "batch_size": 8,  # 要是继续训练, 最好使用相同的batchsize   vgg 8    swins 6   res2_50 24  (训练和测试使用相同的batchsize)
    "num_workers": 0,
    # "num_workers": 6 8 ,  # 不要太大, 不然运行多个程序同时训练的时候, 会造成数据读入速度受影响
    "input_size": 384,
    #     swin 384    vgg/res2_50 256
}
